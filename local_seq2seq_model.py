...
class Seq2SeqModel(object):
  ...
  def __init__(self, source_vocab_size, target_vocab_size, buckets, hidden_size,
               ...
               num_samples=512, forward_only=False, attention=True, sequence_length=None):
    """Create the model.
    ...
    # ttmt update: include model without attention
    def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
      if attention:  
        return local_seq2seq.embedding_attention_seq2seq(
        ...
        return local_seq2seq.embedding_rnn_seq2seq(
          encoder_inputs, decoder_inputs, cell,
          num_encoder_symbols=source_vocab_size,
          num_decoder_symbols=target_vocab_size,
          embedding_size=embedding_size,
    ...
    if forward_only:
      self.outputs, self.losses, self.attns = local_seq2seq.model_with_buckets(
  ...
  def step_with_attn(self, session, encoder_inputs, decoder_inputs, target_weights,
           bucket_id, forward_only):
    """Run a step of the model feeding the given inputs.
    ...
    if not forward_only:
      output_feed = [self.updates[bucket_id],  # Update Op that does SGD.
                     ...
                     self.attns[bucket_id]]  # Loss for this batch.
    else:
      output_feed = [self.losses[bucket_id]]  # Loss for this batch.
      for l in xrange(decoder_size):  # Output logits.
        output_feed.append(self.outputs[bucket_id][l])
      output_feed.append(self.attns[bucket_id])
