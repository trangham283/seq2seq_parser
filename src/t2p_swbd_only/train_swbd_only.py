...
tf.app.flags.DEFINE_string("data_dir", "/tmp/ttran/swbd_data", "Data directory")
...
#train_sw_tmp = pickle.load(open(swtrain_data_path))
# debug with a very small subset:
#train_sw = [train_sw_tmp[0][:200], train_sw_tmp[1][:200], train_sw_tmp[2][:200]]
...
def train():
  ...
  with tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=NUM_THREADS)) as sess:
    ...
    with tf.variable_scope("model", reuse=True):
      ...
      FLAGS.max_gradient_norm, FLAGS.batch_size,
      FLAGS.learning_rate, FLAGS.learning_rate_decay_factor,
      forward_only=True, attention=FLAGS.attention, small_def=True)      
    ...
    while current_step <= num_remaining_steps:
      ...
      for bucket_id, bucket_offset in train_set:
        ...
        # update batch size if at boudary cases,
        if len(this_sample) != model.batch_size: model.batch_size = len(this_sample)
