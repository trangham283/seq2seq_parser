...
def parse_options():
    ...
    parser.add_argument("-lr_decay", "--learning_rate_decay_factor", \
            default=0.95, type=float, help="multiplicative decay factor for learning rate")
    ...
    parser.add_argument("-use_speech", "--use_speech", default=False, \
            action="store_true", help="Use speech features for cnn")
    parser.add_argument("-use_wd", "--use_wd", default=False, \
            action="store_true", help="Use word duration features")
    parser.add_argument("-use_pause", "--use_pause", default=False, \
            action="store_true", help="Use pause features")
    
    # rnn architecture
    parser.add_argument("-text_hsize", "--text_hidden_size", default=256, \
            type=int, help="Hidden layer size of text encoder")
    parser.add_argument("-text_num_layers", "--text_num_layers", default=3, \
    ...
    parser.add_argument("-parse_num_layers", "--parse_num_layers", default=3, \
    ...
    parser.add_argument("-dump_vars", "--dump_vars", default=False, \
            action="store_true", help="Save parameters from the last saved model")
    ...
    if arg_dict['use_speech']:
        ...
        speech_string += 'num_filters' + '_' + str(arg_dict['num_filters']) \
                + '_' + 'filter_sizes' + '_' + str(arg_dict['filter_sizes']) + '_'  
        pool_string = "maxpool_"
        if arg_dict['multipool']:
            pool_string = "multipool_"
    
    pause_string = ""
    if arg_dict['use_pause']:
        pause_string = 'psize' + '_' + str(arg_dict['pause_size']) + '_' 
    ...
    train_dir = ('opt_' + speech_string + pool_string + pause_string + wd_string +
...
def get_model_graph(session, feat_dim, forward_only):
  ...
  model = seqcnn_model.Seq2SeqModel(
      ...
      multipool=FLAGS.multipool, use_pause=FLAGS.use_pause, use_wd=FLAGS.use_wd)
  return model
...
def write_decode(model_dev, sess, dev_set, eval_batch_size, globstep, eval_now=False, prefix=None):  
  # Load vocabularies.
  ...
  gold_file_name = os.path.join(FLAGS.train_dir, prefix+'-gold-step'+ stepname +'.txt')
  print(gold_file_name)
  # file with matched brackets
  decoded_br_file_name = os.path.join(FLAGS.train_dir, prefix+'-br-step'+ stepname +'.txt')
  # file filler XX help as well
  decoded_mx_file_name = os.path.join(FLAGS.train_dir, prefix+'-mx-step'+ stepname +'.txt')
  ...
  for bucket_id in xrange(len(_buckets)):
    ...
    for batch_offset in offsets:
        ...
        text_encoder_inputs, speech_encoder_inputs, pause_bef, pause_aft, \
                ...
                bucket_id, batch_offset, FLAGS.use_speech, FLAGS.use_pause, FLAGS.use_wd)
...
def decode(split='dev', debug=False):
  """ Decode file """
  if split == 'dev':
      dev_set = load_dev_data()
      prefix = 'dev'
  else:
      dev_set = load_test_data()
      prefix = 'test'
  ...
  with tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=NUM_THREADS)) as sess:
    ...
    with tf.variable_scope("model", reuse=None):
      model_dev, steps_done = create_model(sess, feat_dim, forward_only=True, actual_eval=True)
    ...
    write_decode(model_dev, sess, dev_set, eval_batch_size, steps_done, eval_now=True, prefix=prefix) 
