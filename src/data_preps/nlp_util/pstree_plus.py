...
class PSTree:
    ...
    def add_pauses(self, pause_bef, pause_aft):
        terminals = [x for x in self if x.is_terminal()]
        assert len(terminals) == len(pause_bef) == len(pause_aft)
        for i, x in enumerate(terminals):
            x.pause_before = pause_bef[i]
            x.pause_after = pause_aft[i]
