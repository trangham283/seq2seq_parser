...
def select_largest_spans(list_of_spans):
    # find edit node highest in the tree, in case they are nested 
    # if not nested, keep all
    list_of_ranges = [range(x[0], x[1]) for x in list_of_spans]
    edit_sets = [set(x) for x in list_of_ranges]
    high_span_set = []
    for s in edit_sets:
        ss = [x for x in edit_sets if s.issubset(x)]
        max_super = sorted(ss, key = lambda x: len(x))[-1]
        if max_super not in high_span_set:
            high_span_set.append(max_super)
    ans = []
    for s in high_span_set:
        ans.append((min(s), max(s)))
    return ans


def relaxed_counts_for_prf(test_complete, gold_complete, include_root=False, include_terminals=False):
		# remove EDIT nodes to facilitate comparision
		test = test_complete.clone()
		gold = gold_complete.clone()
		from treebanks import remove_nodes
		remove_nodes(test, lambda(t): t.label == 'EDITED', True)
		remove_nodes(gold, lambda(t): t.label == 'EDITED', True)
		tcount = 0
		for node in test:
				if node.is_terminal() and not include_terminals:
						continue
		...
		# Now check if original EDIT nodes matched the spans
		# But just compare the highest nodes, i.e. compare the flattened version
		gold_edit_nodes = [node for node in gold_complete if node.label == 'EDITED']
		test_edit_nodes = [node for node in test_complete if node.label == 'EDITED']
		gold_spans = select_largest_spans([x.span for x in gold_edit_nodes])
		test_spans = select_largest_spans([x.span for x in test_edit_nodes])

		tcount += len(test_spans)
		gcount += len(gold_spans)
		match += len([x for x in test_spans if x in gold_spans])
