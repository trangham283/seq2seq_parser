...
def many2one_attention_seq2seq(
  ...
  with variable_scope.variable_scope(scope or "many2one_attention_seq2seq"):
    ...
    if use_speech:
        ...
        #both_encoder_inputs = [tf.concat(1, \
        #        [text_encoder_inputs[i], speech_conv_outputs[i]]) \
        #        for i in range(encoder_size)]

        # map text and speech features through another linear transform
        comb_size = text_encoder_inputs[0].get_shape()[-1] + \
                speech_conv_outputs[0].get_shape()[-1]
        b_shape = [comb_size]
        W_shape = [comb_size, comb_size] 
        #print(comb_size)
        with variable_scope.variable_scope(scope or "text_speech_comb"):
            W = variable_scope.get_variable("W_comb", W_shape)
            b = variable_scope.get_variable("B_comb", b_shape)
            both_encoder_inputs = [math_ops.matmul(tf.concat(1, \
                    [ text_encoder_inputs[i], speech_conv_outputs[i] ]), W) \
                     for i in range(encoder_size)]
            both_encoder_inputs = [x+b for x in both_encoder_inputs]
    else:
        both_encoder_inputs = text_encoder_inputs

    #print([x.get_shape() for x in both_encoder_inputs])
