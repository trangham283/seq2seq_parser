...
def parse_options():
    ...
    train_dir = ('p3_' + speech_string + pool_string + 
...
def remove_energy(data):
    for bucket_id in range(len(data)):
        for sample_id in range(len(data[bucket_id])):
            data[bucket_id][sample_id][3] = data[bucket_id][sample_id][3][:-1,:]
    return data

def load_test_data():
    test_data_path = os.path.join(FLAGS.data_dir, 'test_p3f4norm_pause.pickle')
    test_set = pickle.load(open(test_data_path))
    test_set = remove_energy(test_set)
    return test_set

def load_dev_data():
    dev_data_path = os.path.join(FLAGS.data_dir, 'dev_p3f4norm_pause.pickle')
    dev_set = pickle.load(open(dev_data_path))
    dev_set = remove_energy(dev_set)
    return dev_set

def load_train_data():
    swtrain_data_path = os.path.join(FLAGS.data_dir, 'train_p3f4norm_pause.pickle')
    train_sw = pickle.load(open(swtrain_data_path))
    train_sw = remove_energy(train_sw)
    sample = train_sw[0][0]
  ...
  with tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=NUM_THREADS)) as sess:
    ...
    while epoch <= FLAGS.max_epochs:
      ...
      for bucket_id, bucket_offset in train_set:
        ...
        text_encoder_inputs, speech_encoder_inputs, pause_bef, pause_aft, decoder_inputs, \
                target_weights, text_seq_len, speech_seq_len = model.get_batch(
                {bucket_id: this_sample}, bucket_id, bucket_offset, FLAGS.use_speech)
        encoder_inputs_list = [text_encoder_inputs, speech_encoder_inputs, pause_bef, pause_aft]
