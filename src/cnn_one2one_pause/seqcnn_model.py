...
class Seq2SeqModel(object):
  ...
  def __init__(self, source_vocab_size, pause_vocab_size, target_vocab_size, 
    ...
    if forward_only:
      self.outputs, self.losses = many2one_seq2seq.many2one_model_with_buckets(
          ...
          softmax_loss_function=softmax_loss_function, spscale=self.spscale, use_speech=use_speech)
  ...
  def get_batch(self, data, bucket_id, bucket_offset, use_speech):
    """Get batches
    ...
    pause_befs, pause_afts = [], []
    sequence_lengths = []

    for sample in data[bucket_id]:
      text_encoder_input, decoder_input, partition, speech_encoder_input, pause_bef, pause_aft = sample
